...
class Tester:
    ...
    def reachable(self, targets, timeout = 0.9):
        result = True
    ...
    def test(self):
        self.logger.debug("Testing reachability")
        if not self.reachable(self.targets) and not self.recording:
            self.start_recording()
            self.logger.debug("Recording time...")
        elif self.recording and self.reachable(self.targets):
            self.stop_recording()
